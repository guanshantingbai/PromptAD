#!/usr/bin/env python3
"""
阶段3：VisA全量训练 - 12个类别 × CLS+SEG × k=1,2,4
"""
...
MAX_WORKERS = 2
OMP_NUM_THREADS = 3
ROOT_DIR = 'result/prompt1_fixed_visa'
...
def main():
    ...
    print(f"\n训练配置：")
    print(f"  数据集: {DATASET}")
    print(f"  类别数: {len(VISA_CLASSES)}")
    print(f"  任务类型: {TASKS}")
    print(f"  K值: {K_SHOTS}")
    print(f"  总任务数: {len(VISA_CLASSES) * len(TASKS) * len(K_SHOTS)}")
    print(f"  并行数: {MAX_WORKERS}")
    print(f"  结果目录: {ROOT_DIR}")
    
    print(f"\nVisA类别列表：")
    for i, cls_name in enumerate(VISA_CLASSES, 1):
        print(f"  {i:2d}. {cls_name}")
    
    total_tasks = len(VISA_CLASSES) * len(TASKS) * len(K_SHOTS)
