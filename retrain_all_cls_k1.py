...
"""
CLS任务 k=1 全类别验证
验证多原型语义分支改进在最具挑战的1-shot设置下的泛化能力
"""
...
def train_single_task(dataset, cls_name, task_id, total_tasks):
    """训练单个CLS k=1任务"""
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 开始任务 {task_id}/{total_tasks}: {dataset}/{cls_name} k=1 (CLS)")
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ 完成: {dataset}/{cls_name} k=1 (CLS) (耗时: {duration:.1f}分钟)")
        ...
        task_end = time.time()
        duration = (task_end - task_start) / 60
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️ 超时: {dataset}/{cls_name} k=1 (CLS) (耗时: {duration:.1f}分钟)")
        return {
            'dataset': dataset,
            'class': cls_name,
            'k': K_SHOT,
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ❌ 失败: {dataset}/{cls_name} k=1 (CLS) (耗时: {duration:.1f}分钟)")
...
def main():
    print("="*80)
    print("CLS任务 k=1 全类别验证")
    print("="*80)
    
    print(f"\n验证目标：")
    print(f"  证明多原型语义分支改进在1-shot CLS任务上的泛化能力")
    print(f"  （SEG任务的语义分支改进效果有限，暂不验证）")
    
    print(f"\n训练配置：")
    print(f"  数据集: MVTec (15类) + VisA (12类)")
    print(f"  任务: 仅CLS (分类)")
    print(f"  K值: 1 (最具挑战的few-shot设置)")
    ...
    # 估算时间
    avg_time_per_task = 2.0  # 每个任务约2分钟
    estimated_time = (total_tasks / MAX_WORKERS) * avg_time_per_task
    
    print(f"\n即将开始训练 {total_tasks} 个CLS k=1任务...")
    ...
    csv_path = f"{ROOT_DIR}/all_cls_k1_results.csv"
    ...
    if failed_count > 0 or timeout_count > 0:
        ...
        for r in results:
            if r['status'] != 'success':
                print(f"\n{r['dataset']}/{r['class']} k=1:")
                print(f"  {r.get('error', 'Unknown error')[:300]}")
    
    print("\n" + "="*80)
    print(f"下一步：运行 python compare_with_baseline.py 查看与baseline的对比")
    print("="*80)
